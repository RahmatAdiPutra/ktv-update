...
use App\Http\Requests\Web\MasterData\SongLanguageRequest;
...
class SongLanguageController extends Controller
{
    ...
    {
        // return $this->seed(10);
        return view('master-data.song-language.index');
    }
    ...
    public function show(SongLanguage $language)
    {
        return $this->responseSuccess($language);
    }

    public function post(SongLanguageRequest $request)
    {
        try {
            if ($request->id) {
                $message = 'Song Language has been updated';
            } else {
                $message = 'Song Language has added';
            }
    ...
    public function destroy(SongLanguage $language)
    {
        $language->delete();
        return $this->responseSuccess(['message' => 'Song Language has been deleted']);
    }
}
